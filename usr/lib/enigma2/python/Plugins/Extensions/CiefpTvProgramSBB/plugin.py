...
try:
    ...
    logging.getLogger("CiefpTvProgramSBB").warning("lxml module not found, attempting to install...")
    try:
        subprocess.run(["pip3", "install", "lxml"], check=True, capture_output=True, text=True)
        from lxml import etree
        LXML_AVAILABLE = True
        logging.getLogger("CiefpTvProgramSBB").info("Successfully installed lxml")
    except Exception as e:
        logging.getLogger("CiefpTvProgramSBB").error(f"Failed to install lxml: {str(e)}. Falling back to ElementTree")
        LXML_AVAILABLE = False

PLUGIN_PATH = "/usr/lib/enigma2/python/Plugins/Extensions/CiefpTvProgramSBB"
EPG_DIR = "/tmp/CiefpTvProgramSBB"
PICON_DIR = os.path.join(PLUGIN_PATH, "picon")
PLACEHOLDER_PICON = os.path.join(PLUGIN_PATH, "placeholder.png")
EPG_URL = "https://epgshare01.online/epgshare01/epg_ripper_RS1.xml.gz"
CACHE_TIME = 86400  # 24 hours caching

# Configure logging for picons and critical errors only
logging.getLogger('').handlers = []
logging.getLogger("CiefpTvProgramSBB").handlers = []
logging.basicConfig(
    level=logging.ERROR,
...
logger = logging.getLogger("CiefpTvProgramSBB")
logger.debug("Initializing CiefpTvProgramSBB logger")

def clean_channel_name(name):
    return ''.join(e.lower() if e.isalnum() or e == '.' else '' for e in name).strip()

class CiefpTvProgramSBB(Screen):
    skin = """
        <screen name="CiefpTvProgramSBB" position="center,center" size="1800,800" title="..:: CiefpTvProgramSBB v1.0 ::..">
...
def main(session, **kwargs):
    session.open(CiefpTvProgramSBB)

def Plugins(**kwargs):
    return [PluginDescriptor(
        name="CiefpTvProgramSBB",
        description="TV Program SBB Srbija EPG plugin,epgshare v1.0",
